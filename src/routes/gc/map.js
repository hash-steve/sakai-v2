...
const WorldMap = () => {
    ...
    const[stakeRewarded, setStakeRewarded] = useState([]);
    const[stakeNotRewarded, setStakeNotRewarded] = useState([]);
    ...
    const handleNodeChange = (response) => {
        const node = response.data.nodes[0];
        setDescription(node.description);
        setNodeAccountId(node.node_account_id);
        setStake(node.stake)
        setStakeRewarded(node.stake_rewarded);
        setStakeNotRewarded(node.stake_not_rewarded);
        setRewardRate(node.reward_rate_start/fn.rateDivisor);
        setMin(node.min_stake);
        setMax(node.max_stake);
    }

    const getNetworkNode = useCallback(async (nodeId) => {        
        await api.getNetworkNode(nodeId).then(response => {
            handleNodeChange(response);
        });
    }, [])
    ...
    return (
        <div className={`card-container`}>
            <div className={`card`}>
                <div className={`sm`}>
                    <div className={`accent nodes title`}></div>
                            ...
                            <table className={`${nodeId===-1 ? 'hidden':''} node-detail p-datatable`} 
                               >
                                ...
                                <tbody className={`p-datatable-tbody`}>                                
                                    ...
                                    <tr className={`p-row-odd`}>
                                        <td>Stake</td>
                                        <td>{Math.round(fn.toHbar(stake)).toLocaleString()}</td>
                                    </tr>
                                    <tr>
                                        <td>Stake Rewarded</td>
                                        <td>{fn.toHbar(stakeRewarded).toLocaleString()}</td>
                                    </tr>
                                    <tr className={`p-row-odd`}>
                                        <td>Stake Not Rewarded</td>
                                        <td>{fn.toHbar(stakeNotRewarded).toLocaleString()}</td>
                                    </tr>
                                    <tr>
                                        <td>Min</td>
                                        <td>{fn.toHbar(min).toLocaleString()}</td>
                                    </tr>
                                    <tr className={`p-row-odd`}>
                                        <td>Max</td>
                                        <td>{fn.toHbar(max).toLocaleString()}</td>
                                    </tr>
                                    <tr>
                                        <td>Reward Rate</td>
                                        <td>{rewardRate}</td>
